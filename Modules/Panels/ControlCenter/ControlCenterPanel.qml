...
SmartPanel {
  ...
  // Get horizontal layout setting from Settings instead of local property
  ...
  preferredWidth: {
    if (!horizontalLayout) {
      return Math.round(430 * Style.uiScaleRatio);
    }
    // In horizontal mode, adjust width based on media card being enabled
    var baseWidth = hasMediaCard ? 800 : 700;
    return Math.round(baseWidth * Style.uiScaleRatio);
  }
  ...
  panelContent: Item {
    ...
    return Math.max(root.shortcutsHeight, root.profileHeight);
      ...
      Loader {
        ...
        visible: active
        anchors.left: profileLoader.top
      ...
      Item {
        ...
        Row {
          ...
          height: {
            ...
            return root.audioHeight;  // Fixed height, no scaling
          }
          spacing: Style.marginL
